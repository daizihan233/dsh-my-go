...
export async function apply(ctx, config = {}) {
  ...
  function cancelDisposeFallback(id) {
    const timer = disposeFallbackTimers.get(id)
    if (timer !== undefined) {
      clearTimeout(timer)
      disposeFallbackTimers.delete(id)
    }
  }
  function scheduleDisposeFallback(id) {
    if (disposeFallbackTimers.has(id)) return
    const timer = setTimeout(() => {
      disposeFallbackTimers.delete(id)
      if (!orchestration.currentMap.has(id)) return
    ...
    timer.unref?.()
    disposeFallbackTimers.set(id, timer)
  }
  ...
  async function loadLedger() {
    try {
      ...
      orchestration.history = list
        .filter((r) => r && typeof r.childId === 'string' && typeof r.agentType === 'string')
        .slice(-200)
      bump()
  ...
  function scheduleLedgerSave() {
    ...
    ledgerSaveTimer = setTimeout(() => {
      ...
      const payload = JSON.stringify({ version: 1, history: orchestration.history.slice(-200) })
  ...
  await loadLedger()
  orchestration.onChange(() => scheduleLedgerSave())
  ...
  function resolveOrchestratorAgent() {
    ...
    return lastOrchestratorSessionId ? agents?.get?.(lastOrchestratorSessionId) : undefined
  }
  // 失败附因兜底：subagent/end 的通知层载荷只有 stopReason 的 kind，
  // error.message 完整存在于子会话档案的 turn/end reason.error——
  // 经 sessions 服务 API 读事件（不手解持久化帧）。读档失败静默退回无附因。
  function readTurnFailure(childId) {
    ...
    } catch { /* 子会话已退出 live store / 服务缺席：静默兜底 */ }
    return undefined
  }
  ...
  function scheduleQueueRetry(work, parentHint, error) {
    work.retries = (work.retries ?? 0) + 1
    if (work.retries > QUEUE_RETRY_MAX) {
      orchestration.dropQueuedFailed(work, error)
  ...
  function advanceQueue(parentHint) {
    ...
    void dispatchWork(work.agentType, work.prompt, parentAgent, undefined, work).catch((error) => {
      orchestration.requeueHead(work)
      bump()
      console.error('[dsh-my-go] queued dispatch failed, task requeued:', error)
      scheduleQueueRetry(work, parentAgent, error)
    })
  }

  async function dispatchWork(agentType, prompt, parent, signal, queuedWork) {
  ...
  ctx.tools.register({
    ...
    async execute(args, exec) {
      ...
      if (!record) throw new Error(`unknown sub-agent id: ${target} — 该 id 不在编排台账；若进程重启过且台账持久化未覆盖该记录（或已被 200 条上限挤出），请用 go_work 重新派发`)
  ...
  ctx.on('subagent/end', (info) => {
    ...
    if (type === undefined) {
      const existing = orchestration.record(childId)
      if (existing) {
        // 台账有归属：以台账为准
        type = existing.agentType
        if (!orchestration.currentMap.has(childId)) {
    ...
    // 失败附因兜底：subagent/end 载荷无 error 字段，经 sessions 服务读子会话
    // 最后一条 turn/end 的 reason.error；读档失败静默退回无附因（不报错）。
    ...
    const done = orchestration.finish(childId, conclusion, failed)
    if (failed && failure) {
      // 失败附因推送：harness 的 settled 通知只带 stopReason，补一行完整原因
      notifyParent(resolveOrchestratorAgent(), `[dsh-my-go] 子代理失败: ${childId} (${type}): ${failure.message} [${failure.code ?? 'UNKNOWN'}]`)
    }
    if (!done) {
  ...
  return () => {
    ...
    // fiber and auto-dispose; manual cleanup covers the queue retry timer,
    // the disposed-grace fallback timers and the ledger debounce timer.
    if (queueRetryTimer) clearTimeout(queueRetryTimer)
    if (ledgerSaveTimer) clearTimeout(ledgerSaveTimer)
    for (const timer of disposeFallbackTimers.values()) clearTimeout(timer)
